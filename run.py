...
from src.linearized import LinearizedLoraTrainer
...
class ModelArguments:
    ...
    #   - prompt-demo: prompt-based fine-tuning with demonstrations (default)
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    trainer: str = field(
        default="linearized",
        metadata={"help": "Pick from {linearized, linearhead}"}
    )
    ...
    binary_classification: bool = field(
        default=False,
        metadata={"help": "If num_classes=2, convert two softmax logits to single sigmoid logit if needed"}
    )
    
    ###################################
    ## New flags for linearized LoRA ##
    ###################################
    
    linear_freeze_A: bool = field(
        default=False,
        metadata={'help': 'freeze layer A'}
    )
    linear_lr: float = field(
        default=0.01,
        metadata={'help': 'lr of linearized lora FT'}
    )
    linear_num_epoch: int = field(
        default=30,
        metadata={'help': 'number of epochs in linearized lora'}
    )
    
    linear_wd: float = field(
        default=0,
        metadata={'help': 'weight dacay in linearized lora'}
    )
    
    eval_during_training: bool = field(
        default=False, 
        metadata={'help': 'Evaluation during the training process'}
    )
    
    train_last_layer: bool = field(
        default=True,
        metadata={'help': 'Train the last layer only'}
    )
...
def main():
    ...
    # # Freeze the marix A if needed
    # if model_args.apply_lora and not model_args.freeze_A_full_lora:
    #     for name, param in model.named_parameters():
    #         if "lora" not in name:
    #             param.requires_grad_(False)
    # elif model_args.apply_lora and model_args.freeze_A_full_lora:
    #     for name, param in model.named_parameters():
    ...
    trainer_class = LinearizedLoraTrainer
    trainer_kwargs = {}
    ...
    if training_args.do_train:
        ...
        total_loss , epoch_count = trainer.finetune(train_dataset, test_dataset)
        ...
        if saved_model_exists and training_args.use_saved_model: #training_args.evaluate_during_training:
    ...
    final_result.update({"avg_train_loss" : total_loss})
    final_result.update({"epoch_count" : epoch_count } )
    eval_results = {}

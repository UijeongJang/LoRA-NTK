...
class Trainer(LinearHeadTrainer):
        ...
        if self.optimizer is None:
            ...
            if self.args.optimizer == 'adam':
                ...
                self.optimizer = SGD(
                    optimizer_grouped_parameters,
                    lr=self.args.learning_rate,
                    weight_decay = self.args.linear_wd,
                )
            else:
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        writer = SummaryWriter(f"./training_standard/{model.module.data_args.task_name}-{self.args.seed}-{model.module.model_args.lora_r}-{model.module.model_args.freeze_A_full_lora}")
        model.zero_grad()
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            for step, inputs in enumerate(tqdm(epoch_iterator, desc=f'Steps in epoch {epoch}', disable=not self.is_local_process_zero())):
                ...
                total_loss += self.training_step(model, inputs) #XXX
            ...
            writer.add_scalar(f"Loss/Train_{model.module.data_args.task_name}",  total_loss/ len(epoch_iterator), epoch)
            if self.args.max_steps > 0 and self.global_step > self.args.max_steps:
            ...
            if self.args.evaluate_during_training:
                ...
                writer.add_scalar(f"Eval_acc_{model.module.data_args.task_name}/epoch", objective , epoch)
